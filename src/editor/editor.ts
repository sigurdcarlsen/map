...
export class Editor {
    ...
    private addEntityToMap(entity: MapEntity, checkRules: boolean = true) {
        ...
        if (entity.type == 'Point') {
            ...
            const marker = L.marker(entity.layer._layers[Object.keys(entity.layer._layers)]._latlng, markerOptions).on(
                'click',
                (e) => {
                    console.log('NOO');
    ...
    private validateSlowly() {
        let validated = 0;
        this.loadingScreenDescription(
            `${this._validateEntitiesQueue.length} entities left for validation by small bureaucrats.<br /><img src="img/hermes.png" height="48px" />`,
        );
